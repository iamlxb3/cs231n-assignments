...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # regularization, it should be dw = dw + 0.5*2*self.reg*W
        dw2 += self.reg*W2
        dw1 += self.reg*W1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
            if i == 0:
                ...
                self.params['W{}'.format(w_b_index)] = np.random.normal(0, weight_scale, input_dim*output_dim).\
                reshape(input_dim, output_dim)
                self.params['b{}'.format(w_b_index)] = np.zeros(output_dim)
            else:
                hidden_index = i - 1
    ...
    def loss(self, X, y=None):
        ...
        cache_dict['cache{}'.format(self.num_layers)] = cache_last
        ...
        # loss
        loss, top_dx = softmax_loss(scores, y)
        loss += 0.5*self.reg*W_reg_sum

        dx_last, dw_last, db_last = affine_backward(top_dx, cache_last) # the derivative of the last layer
        dx = dx_last

        grads['W{}'.format(self.num_layers)] = dw_last
